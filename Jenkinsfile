pipeline {
    ...
    stages {
        ...
        stage('api dev run') {
                steps {
                    git credentialsId: 'cdd4f772-d4c3-473c-9b2a-1056b608a551', url: 'git@github.com:ArthurPopko/pytest_automation.git'
                    sh ''' pytest -v -m api --env dev --alluredir allure-results'''
                }
        }
        stage('data parallel run') {
                steps {
                    git credentialsId: 'cdd4f772-d4c3-473c-9b2a-1056b608a551', url: 'git@github.com:ArthurPopko/pytest_automation.git'
                    sh '''pytest -v -m data -n 4 --env dev --alluredir allure-results'''
                }
        }
        stage('ui dev run') {
                steps {
                    git credentialsId: 'cdd4f772-d4c3-473c-9b2a-1056b608a551', url: 'git@github.com:ArthurPopko/pytest_automation.git'
                    sh '''pytest -v -m 'dev and ui' --env dev --alluredir allure-results'''
                }
        }
        stage('allure') {
            steps {
